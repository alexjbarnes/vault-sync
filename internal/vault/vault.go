...
// maxWriteSize is the maximum content size for MCP write operations.
// This matches the Obsidian sync protocol's default per-file limit
// and prevents memory exhaustion from oversized writes.
const maxWriteSize = 200 * 1024 * 1024 // 200MB
...
func (v *Vault) Write(relPath string, content string, createDirs bool) (*WriteResult, error) {
	...
	if len(content) > maxWriteSize {
		return nil, &Error{
			...
			Message: fmt.Sprintf("content size %d exceeds maximum %d bytes", len(content), maxWriteSize),
		}
	}
...
func (v *Vault) Delete(relPath string) (*DeleteResult, error) {
	...
	return v.deleteLocked(relPath)
}

// deleteLocked performs the actual delete without acquiring the mutex.
// The caller must hold v.mu.
func (v *Vault) deleteLocked(relPath string) (*DeleteResult, error) {
	if err := validatePath(relPath); err != nil {
		return nil, err
	}
