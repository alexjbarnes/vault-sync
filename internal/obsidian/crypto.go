...
func KeyHashV3(key []byte, salt string) (string, error) {
	...
	derived, err := hkdfDeriveKey(key, saltBytes, []byte("ObsidianKeyHash"), 32)
	if err != nil {
...
func NewCipherV3(key []byte, salt string) (*CipherV3, error) {
	...
	encKey, err := hkdfDeriveKey(key, saltBytes, []byte("ObsidianAesSivEnc"), 32)
	if err != nil {
		return nil, fmt.Errorf("deriving SIV enc key: %w", err)
	}

	// siv.NewCMAC expects a 64-byte composite key: [mac_key(32) ‖ enc_key(32)].
	sivKey := append(macKey, encKey...) //nolint:gocritic
	sivCipher, err := siv.NewCMAC(sivKey)
	if err != nil {
		return nil, fmt.Errorf("creating AES-SIV cipher: %w", err)
	}

	gcmKey, err := hkdfDeriveKey(key, nil, []byte("ObsidianAesGcm"), 32)
	if err != nil {
