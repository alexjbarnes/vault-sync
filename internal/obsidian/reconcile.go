...
// Phase1 executes the full reconciliation sequence:
...
func (r *Reconciler) threeWayMerge(ctx context.Context, path string, push PushMessage, local state.LocalFile, prev state.ServerFile, hasPrev bool) error {
	...
	if patchFailed {
		...
		cp := conflictCopyPath(base, ext)

		if err := r.vault.WriteFile(cp, localContent, time.Time{}); err != nil {
			r.logger.Warn("reconcile: failed to save conflict copy",
				slog.String("path", cp),
				slog.String("error", err.Error()),
			)
		} else {
			r.logger.Info("reconcile: saved conflict copy before merge",
				slog.String("path", cp),
			)
		}
	}
